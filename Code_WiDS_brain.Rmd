...
process_connectomes <- function(folder_path) {
  ...
  data <- tsv_files %>%
    ...
    map_df(~ {
      mat <- as.matrix(read_tsv(.x, col_names = FALSE))
      tibble(id = tools::file_path_sans_ext(basename(.x)),
             features = list(extract_upper_triangle(mat)))
    })
  
  # Turn the list-column into a wide format dataframe
  data_features <- data %>%
...
train_path <- "~/Desktop/data/train_tsv/train_tsv"
test_path <- "~/Desktop/data/test_tsv/test_tsv"

train_df <- process_connectomes(train_path)
test_df  <- process_connectomes(test_path)

train_metadata <- read_csv("~/Desktop/data/metadata/training_metadata.csv")
test_metadata <- read_csv("~/Desktop/data/metadata/test_metadata.csv")
